...
#SBATCH --output=Logs/dedup_%j.out       #optional: file to store stdout from job, %j adds the assigned jobID
#SBATCH --error=Logs/dedup_%j.err        #optional: file to store stderr from job, %j adds the assigned jobID



#samtools sort -O sam /projects/bgmp/shared/deduper/C1_SE_uniqAlign.sam -o C1_SE_uniqAlign_sorted.sam

/usr/bin/time -v ./Hays_deduper_randomers.py -f C1_SE_uniqAlign_sorted.sam -o dedup_out.sam
