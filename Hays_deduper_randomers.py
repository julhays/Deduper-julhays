...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument("-u", "--umi", help="designates file path containing the list of UMIs", type=str, default="randomer" )
    ...
    with open(output, "wt") as out:
        ...
        for line in fh:
            ...
            #all other lines
                ...
                #if UMI list not given, check to see if UMI is valid randomer, if not discard
                if umi_file_path == 'randomer':
                    valid = True
                    if len(umi) != 8:
                        valid = False
                    for base in umi:
                        if base not in set('ATGCatcg'):
                            valid = False
                    if valid == False:
                        unknown_umi_count += 1
                        continue
        
                #if UMI list given check if UMI in list of known UMIs, if its not discard    
                else:
